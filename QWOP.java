  ...
  private boolean dead;
    ...
    add(floor);
	 ...
	 @Override
      ...
      double backKneeAngle = PI - (backThigh.getAngle() - backCalf.getAngle());
      double frontKneeAngle = PI - (frontThigh.getAngle() - frontCalf.getAngle());
      ...
      if (backKneeAngle > MAX_KNEE_ANGLE) {
	    ...
	    if(frontCalf.getEnd().getY() >= floor.getY() ||
	      ...
	      //front foot on ground
	        ...
	        if (last != Case.FRONT) countSame = 0;
          else countSame ++;
	        ...
	        backThigh.update(false);
	        backCalf.increaseAngVelocity(.005*(backThigh.getAngle() - 1.3*backCalf.getAngle()));
	        backCalf.update();
          last = Case.FRONT;
	      }
	      //back foot on ground
	      else {
	        if (last != Case.BACK) countSame = 0;
          else countSame ++;
	        ...
	        frontThigh.increaseAngVelocity(angularAccel);
	        frontThigh.update(false);
          frontCalf.increaseAngVelocity(.005*(frontThigh.getAngle() - 1.3*frontCalf.getAngle()));
	        frontCalf.update();
          last = Case.BACK;
	      }
	    }
